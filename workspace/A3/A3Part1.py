...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    # test inputs:
    # real: x = np.cos(2*np.pi*f1/fs*np.arange(N)) + np.cos(2*np.pi*f2/fs*np.arange(N))
    # complex: x = np.exp(1j*2*np.pi*f1/fs*np.arange(N)) + np.exp(1j*2*np.pi*f2/fs*np.arange(N))
