...
def zpFFTsizeExpt(x, fs):
    ...
    """ Plot all 3 magnitude spectrums on same plot

    plt.plot(float(fs)*np.arange(mX1.size)/float(M), mX1, 'r')
    plt.plot(float(fs)*np.arange(mX2.size)/float(2*M), mX2, 'r')
    plt.plot(float(fs)*np.arange(mX3.size)/float(2*M), mX3, 'r')

    mXall = np.array([])
    mXall = np.append(mXall, mX1)
    mXall = np.append(mXall, mX2)
    mXall = np.append(mXall, mX3)

    plt.axis([0, fs/2.0, min(mXall), max(mXall)])
