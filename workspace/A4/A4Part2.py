...
def computeSNR(inputFile, window, M, N, H):
    ...
    # peform stft analysis/synthesis
    y = stft.stft(x, fs, w, N, H)
    noise = y - x

    # compute SNR for entire signal
    E_signal = np.square(x).sum()
    E_noise = np.square(noise).sum()
    SNR1 = 10 * np.log10(E_signal / E_noise)

    # discard M samples from start and end
    x2 = x[M:-M]
    noise2 = noise[M:-M]

    # compute SNR for x2 and y2
    E_signal2 = np.square(x2).sum()
    E_noise2 = np.square(noise2).sum()
