...
def computeODF(inputFile, window, M, N, H):
    ...
    # compute difference across energy envelopes
    # axis=0 to compute difference accross frames and not bands
    ODF = np.diff(engEnv, axis=0)
    # half-wave rectification
    ODF[ODF < 0] = 0

    # insert zeros at 0th frame for both bands (since ODF starts at 1)
    ODF = np.concatenate((np.zeros((1, 2)), ODF))
