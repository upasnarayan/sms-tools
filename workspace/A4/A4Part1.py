...
def extractMainLobe(window, M):
    ...
    # size of fft buffer
    N = 8*M
    # zero padding + zero-phase windowing
    ...
    # undo zero-phase windowing by using fftshift()
    mX = fftshift(mX)

    # compute local minima of right side of main lobe
    right = -1
    for i in np.arange(N/2, N-1, 1):
        curr = mX[i]
        next = mX[i+1]
        if next > curr:
            right = i
    ...
    # compute local minima of left side of main lobe
    left = -1
