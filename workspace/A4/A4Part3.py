...
def computeEngEnv(inputFile, window, M, N, H):
    ...
    # peform stft analysis
    xmXdB, xpX = stft.stftAnal(x, fs, w, N, H)
    
    # convert mX from dB to linear
    xmX = np.power(10, xmXdB / 20)

    # find index of 0, 3000, and 10000 frequencies
    # frequency = k * fs / N
    # k = frequency * N / fs
    k0 = 1
    k3L = np.floor(3000.0 * N / fs)
    k3H = np.ceil(3000.0 * N / fs)
    k10 = np.floor(10000.0 * N / fs)
    
    # compute energy envelopes
    # sum over frequency axis (axis = 1)
    engLow = np.square(xmX[:, k0:k3L+1]).sum(axis=1)
    engLowdB = 10 * np.log10(engLow)
    engHigh = np.square(xmX[:, k3H:k10+1]).sum(axis=1)
    engHighdB = 10 * np.log10(engHigh)

    # stack together low and high envelopes
    engEnv = np.column_stack((engLowdB, engHighdB))
    return engEnv


    """
    Alternative (less efficient) method: loop through frames
        and compute energy envelopes at each frame

    for i in np.arange(xmX.shape[0]):

        # energy envelope for low band
        engLow = np.square(xmX[i, k0:k3L+1]).sum()
        engLowdB = 10 * np.log10(engLow)
        # energy envelope for high band
        engHigh = np.square(xmX[i, k3H:k10+1]).sum()
        engHighdB = 10 * np.log10(engHigh)
        
        # array to vstack (engEnv for frame i)
        engEnvi = [engLowdB, engHighdB]

        if i == 0:
            engEnv = np.array(engEnvi)
        else:
            engEnv = np.vstack((engEnv, engEnvi))
    """