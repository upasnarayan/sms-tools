...
def minFreqEstErr(inputFile, f):
    ...
    M = 101
    # iterate through all M less than input signal
    while M <= x.size:
        # get a blackman window of size M
        w = get_window(window, M)
        # get portion of input centered at 0.5
        x1 = x[int(0.5*fs-M/2):int(0.5*fs+M/2)+1]
        # compute N as power of 2 larger than M
        N = int(np.power(2, np.ceil(np.log2(M))))
        # perform DFT analysis of windowed input
        mX, pX = DFT.dftAnal(x1, w, N)
        # get locations from peak detection and interpolate
        ploc = UF.peakDetection(mX, t)
        iploc, ipmag, ipphase = UF.peakInterp(mX, pX, ploc)
        # convert interpolated locations to frequencies
        freqs = iploc * fs / float(N)

        # check whether difference between freqs and f is less than 0.05 Hz
        diff = abs(freqs - f)
        thresh = 0.05
        validFreqs = freqs[diff < thresh]
        if validFreqs.size > 0:
            # just take the first freq found that satisfies the threshold
            fEst = float(validFreqs[0])
            return (fEst, M, N)

        # increase M by 100 for next iteration
        M += 100 