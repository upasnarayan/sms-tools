...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    l1 = np.ceil(t1 * fs / float(H))
    l2 = np.floor(t2 * fs / float(H))
    hfseg = hfreq[l1:l2+1,:]

    # 3. Compute the mean inharmonicity of the segment
    numFrames = hfseg.shape[0]
    numHarmonics = hfseg.shape[1]
    inharm = np.zeros(numFrames)

    # iterate over all frames
    for l in np.arange(numFrames):
        err = 0
        # fundamental frequency is 1st harmonic (at frame index 0)
        f0l = hfseg[l, 0]
        # compute the actual number of harmonics in R
        R = 0
        # iterate over all harmonics
        for r in np.arange(numHarmonics):
            # discard any harmonics with frequency <= eps
            if hfseg[l,r] > eps:
                # use (r+1) because of 0-indexing
                err += abs(hfseg[l,r] - (r+1)*f0l) / float(r+1)
                R += 1
        inharm[l] = err / float(R)

    # compute mean inharmonicity
    meanInharm = 1 / float(l2 - l1 + 1) * np.sum(inharm)
