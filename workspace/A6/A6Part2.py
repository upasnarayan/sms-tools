...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    regions = np.array([])
    for i in range(winStable-1, f0cents.size):
        stdev = np.std(f0cents[i-(winStable-1):i+1])
        if stdev < stdThsld:
    ...
    #5. apply segment filtering, i.e. remove segments with are < minNoteDur in lengthsegments = np.array([])
    segments = np.array([])
    start = 0
    # go through all frames
    for i in range(0, regions.size-1):
        # check if frames are consecutive
        if regions[i+1] != regions[i]+1:
            # compute duration of segment in seconds
            # end frame - start frame + 1 since start/end frames are inclusive
            duration = (regions[i]-regions[start]+1) / float(fs) * H
            # only add to segments if duration is at least minNoteDur
            if duration >= minNoteDur:
                currSegment = [regions[start], regions[i]]
                # create array if size is 0, else stack segments vertically
                if segments.size == 0:
                    segments = np.array([currSegment])
                else:
                    segments = np.vstack((segments, np.array([currSegment])))
            # start again from next frame
            start = i+1
